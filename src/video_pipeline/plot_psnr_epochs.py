...
def parse_epochs_from_filename(path):
    m = re.search(r"results_(\d+)_epochs\.csv$", path)
    if m:
...
def find_result_csvs(root_dir="."):
    pattern = os.path.join(root_dir, "results/results_*_epochs.csv")
...
def plot_all_videos_single_plot(videos_dict, out_dir="plots"):
    ...
    plt.title("PSNR vs epochs with Scaling of 4 — sources/ videos")
    plt.xlabel("epochs")
